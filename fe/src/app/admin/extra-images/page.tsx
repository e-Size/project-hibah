...
import type { ExtraImage, ExtraImageUpdateRequest, PaginationMeta } from "@/types/admin";
...
export default function ExtraImagesPage() {
  ...
  const fetchImages = useCallback(async () => {
    setLoading(true);
    try {
      const res = await extraImageService.getPaginated({ page, search });
      setImages(res.data);
      setMeta(res.meta);
    } catch { showToast("Gagal mengambil data gambar tambahan", "error"); }
    setLoading(false);
  }, [page, search]);
  ...
  const handleSave = async () => {
    if (!name.trim()) { showToast("Nama gambar wajib diisi", "error"); return; }
    if (!editingId && !imageFile) { showToast("File gambar wajib diupload", "error"); return; }
    setIsSaving(true);
    try {
      let uploadedUrl = "";
      ...
      handleCloseModal();
      fetchImages();
    } catch { showToast("Gagal menyimpan gambar", "error"); }
    setIsSaving(false);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      fetchImages();
    } catch { showToast("Gagal menghapus gambar", "error"); }
    setIsSaving(false);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    setIsSaving(true);
    try {
      await Promise.all(Array.from(selectedImages).map(id => extraImageService.delete(id)));
      showToast(`${selectedImages.size} gambar berhasil dihapus`, "success");
      setBulkDeleting(false);
      setSelectedImages(new Set());
      fetchImages();
    } catch { showToast("Beberapa gambar mungkin gagal dihapus", "error"); fetchImages(); }
    setIsSaving(false);
  };
  ...
  return (
    <>
        ...
        <div className="admin-card-body">
          ...
          {images.length > 0 && (
            <div style={{ padding: "0 0 16px 0", display: "flex", alignItems: "center", gap: 8 }}>
              <input type="checkbox" checked={selectedImages.size === images.length} onChange={toggleSelectAll} style={{ cursor: "pointer" }} />
              ...
              {images.map((img) => (
                <div key={img.id} style={{ position: "relative", borderRadius: 12, overflow: "hidden", border: `1px solid ${selectedImages.has(img.id) ? "var(--admin-primary)" : "var(--admin-border)"}`, background: "#f8fafc", boxShadow: selectedImages.has(img.id) ? "0 0 0 2px var(--admin-primary) inset" : "none" }}>
                  <div style={{ position: "absolute", top: 8, left: 8, zIndex: 10 }}>
                    <input type="checkbox" checked={selectedImages.has(img.id)} onChange={() => toggleSelect(img.id)} style={{ cursor: "pointer", width: 16, height: 16 }} />
                  </div>
                  <div style={{ cursor: "pointer", width: "100%", height: 160 }} onClick={() => setViewingImage(img)} title="Klik untuk melihat">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={`${API_HOST}${img.image_url}`} alt={img.name} style={{ width: "100%", height: "100%", objectFit: "contain", display: "block", background: "white" }} />
                  </div>
                  <div style={{ padding: "12px 16px", borderTop: "1px solid var(--admin-border)", background: "white" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 4 }}>
                      ...
                      <div style={{ display: "flex", gap: 4 }}>
                        <button className="admin-btn admin-btn-ghost admin-btn-icon" style={{ color: "#64748b", padding: 4 }} onClick={() => handleOpenModal(img)} title="Edit">
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="14" height="14"><path d="M5.433 13.917l1.262-3.155A4 4 0 017.58 9.42l6.92-6.918a2.121 2.121 0 013 3l-6.92 6.918c-.383.383-.84.685-1.343.886l-3.154 1.262a.5.5 0 01-.65-.65z" /><path d="M3.5 5.75c0-.69.56-1.25 1.25-1.25H10A.75.75 0 0010 3H4.75A2.75 2.75 0 002 5.75v9.5A2.75 2.75 0 004.75 18h9.5A2.75 2.75 0 0017 15.25V10a.75.75 0 00-1.5 0v5.25c0 .69-.56 1.25-1.25 1.25h-9.5c-.69 0-1.25-.56-1.25-1.25v-9.5z" /></svg>
                        </button>
                        <button className="admin-btn admin-btn-ghost admin-btn-icon" style={{ color: "var(--admin-danger)", padding: 4 }} onClick={() => setDeleteTarget(img)} title="Hapus">
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="14" height="14"><path fillRule="evenodd" d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.841 10.518A2.75 2.75 0 007.596 19h4.807a2.75 2.75 0 002.742-2.53l.841-10.52.149.023a.75.75 0 00.23-1.482A41.03 41.03 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 00-1.5.06l.3 7.5a.75.75 0 101.5-.06l-.3-7.5zm4.34.06a.75.75 0 10-1.5-.06l-.3 7.5a.75.75 0 101.5.06l.3-7.5z" clipRule="evenodd" /></svg>
                        </button>
                      </div>
                    </div>
                  </div>
      ...
      >
        <div onKeyDown={(e) => { if (e.key === "Enter") handleSave(); }}>
          <div className="admin-form-group">
            ...
            <input type="text" className="admin-form-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Contoh: Banner Promo, Panduan Ukuran..." required />
          </div>
          <div className="admin-form-group">
            <label className="admin-form-label">Deskripsi (Opsional)</label>
            <textarea className="admin-form-textarea" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Contoh: Digunakan di halaman utama bagian atas" rows={2} />
          </div>
          <div className="admin-form-group">
      ...
      {viewingImage && (
        ...
        >
          <button style={{ position: "absolute", top: 20, right: 20, background: "white", border: "none", borderRadius: "50%", width: 40, height: 40, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setViewingImage(null)}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="24" height="24"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
          </button>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={`${API_HOST}${viewingImage.image_url}`} alt="View" style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain", borderRadius: 8, background: "white" }} onClick={(e) => e.stopPropagation()} />
        </div>
      )}

      {deleteTarget && (
        <DeleteConfirm isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} loading={isSaving} itemName={`gambar "${deleteTarget.name}"`} />
      )}
      <DeleteConfirm isOpen={bulkDeleting} onClose={() => setBulkDeleting(false)} onConfirm={handleBulkDelete} loading={isSaving} itemName={`${selectedImages.size} gambar terpilih`} />
    </>
  );
}
