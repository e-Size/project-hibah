...
import type { ProductImage, ProductListItem, PaginationMeta } from "@/types/admin";
...
export default function ProductImagesPage() {
  ...
  const loadImages = useCallback(async () => {
    ...
    try {
      const res = await productImageService.getPaginated({ page, product_id: selectedProduct || undefined });
      setImages(res.data);
      setMeta(res.meta);
  ...
  const handleFilterChange = (v: string) => { setSelectedProduct(v); setPage(1); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setModalOpen(false);
      loadImages();
    } catch { showToast(isEditMode ? "Gagal update gambar" : "Gagal upload gambar", "error"); }
    setSaving(false);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    setSaving(true);
    try {
      await Promise.all(Array.from(selectedImages).map(id => productImageService.delete(id)));
      showToast(`${selectedImages.size} gambar berhasil dihapus`, "success");
      setBulkDeleting(false);
      setSelectedImages(new Set());
      loadImages();
    } catch { showToast("Beberapa gambar mungkin gagal dihapus", "error"); loadImages(); }
    setSaving(false);
  };
  ...
  return (
    <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 16 }}>
              {images.map((img) => (
                <div key={img.id} style={{ position: "relative", borderRadius: 12, overflow: "hidden", border: `1px solid ${selectedImages.has(img.id) ? "var(--admin-primary)" : "var(--admin-border)"}`, background: "#f8fafc", boxShadow: selectedImages.has(img.id) ? "0 0 0 2px var(--admin-primary) inset" : "none" }}>
                  ...
                  <div style={{ cursor: "pointer", width: "100%", height: 140 }} onClick={() => setViewingImage(img)} title="Klik untuk melihat">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={`${API_HOST}${img.url}`} alt="" style={{ width: "100%", height: "100%", objectFit: "contain", display: "block", background: "white" }} />
                  </div>
                  <div style={{ padding: "8px 12px", borderTop: "1px solid var(--admin-border)", background: "white" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <span style={{ fontSize: "0.75rem", color: "var(--admin-text-secondary)", fontWeight: 500 }}>Order: {img.order}</span>
                      <div style={{ display: "flex", gap: 4 }}>
                        <button className="admin-btn admin-btn-ghost admin-btn-icon" style={{ color: "#64748b", padding: 4 }} onClick={() => openEdit(img)} title="Edit">
                        ...
                        <button className="admin-btn admin-btn-ghost admin-btn-icon" style={{ color: "var(--admin-danger)", padding: 4 }} onClick={() => setDeleting(img)} title="Hapus">
      ...
      >
        <div onKeyDown={(e) => { if (e.key === "Enter") handleSave(); }}>
          <div className="admin-form-group">
            ...
            <select className="admin-form-select" value={uploadProduct} onChange={(e) => setUploadProduct(e.target.value)} disabled={isEditMode} style={{ background: isEditMode ? "#f1f5f9" : undefined }}>
          ...
          <div className="admin-form-group">
            ...
            <p className="admin-form-hint">Format: JPG, PNG, WebP {isEditMode && "— Kosongkan jika tidak ingin ganti gambar"}</p>
          </div>
          {previewUrl && (
      ...
      {viewingImage && (
        <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", padding: 40 }} onClick={() => setViewingImage(null)}>
          ...
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={`${API_HOST}${viewingImage.url}`} alt="View" style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain", borderRadius: 8, background: "white" }} onClick={(e) => e.stopPropagation()} />
        </div>
      )}

      <DeleteConfirm isOpen={!!deleting} onClose={() => setDeleting(null)} onConfirm={handleDelete} loading={saving} itemName="gambar" />
      <DeleteConfirm isOpen={bulkDeleting} onClose={() => setBulkDeleting(false)} onConfirm={handleBulkDelete} loading={saving} itemName={`${selectedImages.size} gambar terpilih`} />
    </>
  );
}
