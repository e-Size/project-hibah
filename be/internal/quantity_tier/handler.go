...
func (h *Handler) Update(c *gin.Context) {
	...
	t, err := h.service.Update(c.Param("id"), req)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			c.JSON(http.StatusNotFound, gin.H{"error": "Quantity tier not found"})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
...
func (h *Handler) Delete(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "Quantity tier deleted"})
}
